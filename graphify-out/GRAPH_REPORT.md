...
## Corpus Check
- 35 files · ~34,267,134 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 678 nodes · 1426 edges · 22 communities detected
...
- `reason_after_mapping()` --calls--> `append()`  [INFERRED]
  /tmp/etroc-visual-no-ball-report-plan/update_second_batch_xray_mapping.py → hybrid-bbqc/etroc_position_reviews.py
- `reason_after_mapping()` --calls--> `append()`  [INFERRED]
  /tmp/etroc-visual-no-ball-report-plan/update_second_batch_xray_mapping.py → hybrid-bbqc/etroc_reviews.py
- `xray_crop_article()` --calls--> `append()`  [INFERRED]
  /tmp/etroc-visual-no-ball-report-plan/update_second_batch_xray_mapping.py → hybrid-bbqc/etroc_position_reviews.py
...
Cohesion: 0.06
Nodes (75): applyQueue(), canonicalComparator(), canonicalMontageUri(), canSaveActive(), closeDialog(), controls(), controlsEnabled(), currentDraft() (+67 more)
...
Cohesion: 0.06
Nodes (42): _asset_bytes(), audit(), _checksum_inventory(), _completion(), completion_summary(), _current(), _digest(), _drop_empty_legacy_schema() (+34 more)

### Community 3 - "Community 3"
Cohesion: 0.07
Nodes (40): analyze_one(), grid_centers(), grid_scores(), main(), optical_labels(), overlay(), _peaks(), read_manifest() (+32 more)
...
Cohesion: 0.08
Nodes (21): appendCell(), element(), initStatistics(), initStatisticsTableSorting(), parseVerifiedPublication(), ratioLabel(), renderCategoryRows(), renderStatistics() (+13 more)

### Community 6 - "Community 6"
Cohesion: 0.12
Nodes (18): atomic_replace_directory(), build_pool(), height_publication(), int_field(), load_height_contract(), main(), position_publication(), render_clean_montage() (+10 more)
...
Cohesion: 0.21
Nodes (16): apply(), categoryStrip(), clear(), counts(), createViewer(), exact(), node(), openResultViewer() (+8 more)
...
Cohesion: 0.67
Nodes (1): Execute the real result viewer/controller in a small native-DOM boundary harness

## Knowledge Gaps
- **5 isolated node(s):** `Bounded current labels from one read snapshot, never history or review notes.`, `Normalize API-defaulted empty containers without hiding non-empty drift.`, `Execute the real result viewer/controller in a small native-DOM boundary harness`, `Release closure for the result-first ETROC entry document.`, `Behavioral regressions for the post-review save/readback UI boundary.`
...
- **Thin community `Community 19`** (6 nodes): `HybridPairCorrectionTests`, `.test_concordance_snapshot_uses_the_corrected_comment_target()`, `.test_corrected_detail_page_keeps_an_old_url_redirect()`, `.test_dashboard_uses_only_the_corrected_w04f2_81_slug()`, `.test_manifest_preserves_w04f2_34_and_corrects_w04f2_81()`, `test_hybrid_pair_correction.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (4 nodes): `Behavioral regressions for the post-review save/readback UI boundary.`, `test_actual_position_save_refreshes_only_validated_readback()`, `test_result_style_selectors_and_pending_copy_match_runtime()`, `test_etroc_postreview_regressions.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (3 nodes): `Execute the real result viewer/controller in a small native-DOM boundary harness`, `test_read_only_result_viewer()`, `test_etroc_result_viewer.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `append()` connect `Community 3` to `Community 0`, `Community 2`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 13`, `Community 14`?**
  _High betweenness centrality (0.341) - this node is a cross-community bridge._
- **Why does `LxplusDashboardDeployTests` connect `Community 1` to `Community 9`, `Community 3`?**
  _High betweenness centrality (0.196) - this node is a cross-community bridge._
- **Why does `append()` connect `Community 3` to `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 9`, `Community 12`, `Community 14`?**
  _High betweenness centrality (0.192) - this node is a cross-community bridge._
- **Are the 48 inferred relationships involving `append()` (e.g. with `.handle_data()` and `.handle_endtag()`) actually correct?**
  _`append()` has 48 INFERRED edges - model-reasoned connections that need verification._
...
- **What connects `Bounded current labels from one read snapshot, never history or review notes.`, `Normalize API-defaulted empty containers without hiding non-empty drift.`, `Execute the real result viewer/controller in a small native-DOM boundary harness` to the rest of the system?**
