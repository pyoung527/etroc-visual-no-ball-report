(() => {
  ...
  function reconcilePositionEvidence(record, parsed, summary, expectedPublicationSha256) {
    ...
    Object.entries(summary.reviews).forEach(([key, review]) => {
      ...
      if (!evidence || !review || POSITION_KEY_FIELDS.some((field) => review[field] !== evidence[field]) || !HUMAN_LABELS.includes(review.label)
  ...
  function applyQueue(record, origin) { const opened = openQueue(state.records, {}, record, filters()); state.mode = opened.mode; state.queue = opened.queue; state.queueIndex = opened.queueIndex; state.queueFilters = opened.filters; state.active = state.queue[0]; state.origin = origin; dialog.hidden = false; updateDialog(); close.focus(); }
  ...
  function updateCardReview(card, record) { if (!card) return; let detail = card.querySelector("[data-etroc-review-card-state]"); if (!detail) { detail = document.createElement("small"); detail.setAttribute("data-etroc-review-card-state", ""); card.querySelector(".etroc-pool-card-body")?.append(detail); } detail.textContent = `${record.needs_inspection_count} NEED_INSPECT position${record.needs_inspection_count === 1 ? "" : "s"} · open to classify`; }
  function updateReviewSurface() { if (!state.summary) return; let visible = 0; state.records.forEach((record) => { const matches = candidateMatch(record, candidateFilter?.value || "needs_inspection"); const card = document.querySelector(`[data-etroc-acquisition="${record.acquisition_id}"]`); card?.classList.toggle("review-filter-hidden", !matches); updateCardReview(card, record); visible += Number(matches); }); document.querySelectorAll(".etroc-pool-group").forEach((group) => { const cards = [...group.querySelectorAll(".etroc-pool-card")]; const shown = cards.filter((card) => !card.classList.contains("review-filter-hidden")).length; const label = group.querySelector(".wafer span"); if (label) label.textContent = `${shown} / ${cards.length} ETROCs shown · ETROC_OI_2608 exploratory montages`; }); summaryNode.textContent = `${visible} / ${state.records.length} ETROCs shown · classify NEED_INSPECT positions as GREEN / BLUE / YELLOW / RED`; const queue = makeQueue(state.records, {}, { ...filters(), state: "" }); start.disabled = !state.verified || !queue.length; }
