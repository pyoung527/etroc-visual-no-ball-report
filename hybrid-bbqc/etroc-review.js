(() => {
  ...
  function ensureWorkspaceControls() {
    ...
    if (reviewControls && !reviewControls.querySelector("[data-etroc-candidate-filter]")) {
      ...
      [["all", "All ETROCs"], ["needs_inspection", "Has NEED_INSPECT targets"], ["no_ball", "Optical no-ball"], ["red", "Red"], ["review_candidate", "Review candidate"]].forEach(([value, text]) => { const option = document.createElement("option"); option.value = value; option.textContent = text; select.append(option); });
    ...
    if (reviewControls && !reviewControls.querySelector("[data-etroc-review-completion-filter]")) {
      ...
      [["all", "All review states"], ["pending", "Review pending"], ["complete", "Review complete"]].forEach(([value, text]) => { const option = document.createElement("option"); option.value = value; option.textContent = text; select.append(option); });
      label.append(select); reviewControls.insertBefore(label, reviewControls.querySelector("[data-etroc-review-filter-order]")?.closest("label") || null);
    }
    ...
    if (!evidence.querySelector("[data-etroc-review-scientific-context]")) {
      ...
      heading.textContent = "Original evidence — algorithm screening context";
  ...
  async function savePositionReview(andNext) {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        if (conflict.code === "stale_current" && state.position.conflict.current) globalThis.dispatchEvent(new CustomEvent("etroc-position-review-updated"));
      ...
      updateReviewSurface();
      const current = summary.reviews[String(active.position)]; if (!Number.isSafeInteger(body.event.event_id) || body.event.event_id <= 0 || !current || !Number.isSafeInteger(current.current_event_id) || current.current_event_id < body.event.event_id || (current.current_event_id === body.event.event_id && current.label !== draft.label) || POSITION_KEY_FIELDS.some((field) => current[field] !== active[field])) throw new Error("position save readback mismatch");
      const history = await fetchPositionHistory(state.active, active.position);
      const items = history.history;
      const savedIndex = Array.isArray(items) ? items.findIndex((item) => item?.event_id === body.event.event_id) : -1;
      if (savedIndex < 0 || items[0]?.event_id !== current.current_event_id || items[0].label !== current.label || items.slice(0, savedIndex + 1).some((item, index) => !item || !Number.isSafeInteger(item.event_id) || item.event_id <= 0 || POSITION_KEY_FIELDS.some((field) => item[field] !== active[field]) || (index < savedIndex && (item.supersedes_event_id !== items[index + 1]?.event_id || item.event_id <= items[index + 1]?.event_id))) || ["label", "note", "mutation_id", "supersedes_event_id"].some((field) => items[savedIndex][field] !== body.event[field])) throw new Error("position history readback mismatch");
      setPositionHistory(history.history); renderPositionGrid(); updatePositionProgress();
      globalThis.dispatchEvent(new CustomEvent("etroc-position-review-updated"));
  ...
  // Result cards and filters are owned exclusively by etroc-results.js.
  // Event delegation keeps correction actions alive after result refreshes.
  document.addEventListener("click", (event) => {
    const origin = event.target.closest("[data-etroc-inspect]");
    if (!origin) return;
    const record = state.records.find(item => item.acquisition_id === origin.dataset.etrocInspect);
    if (!state.verified || !record) {
      const message = document.querySelector("[data-etroc-results-status]");
      if (message) message.textContent = "Evidence review workspace unavailable. Refresh results or reload to retry.";
      return;
    }
    applyQueue(record, origin);
  });
  function updateReviewSurface() {
    if (!state.summary || !state.completion.size) return;
    const queue = makeCompletionQueue(state.records, state.completion, filters());
    const reviewed = [...state.completion.values()].reduce((total, item) => total + item.reviewed_target_count, 0);
    const targets = state.records.reduce((total, item) => total + item.position_review_target_count, 0);
    summaryNode.textContent = `${reviewed}/${targets} target positions labelled · ${queue.length} ETROCs in this pending queue`;
    start.disabled = !state.verified || !queue.length;
  }

  globalThis.addEventListener("etroc-optical-publication", async (event) => { try { const detail = event.detail || {}; const parsed = detail.records && detail.publicationSha256 ? publicationFromEvent(detail) : await parsePublication(detail.bytes); const [summary, completionPayload] = await Promise.all([fetchSummary(), fetchCompletionSummary()]); state.evidence = reconcileEvidence(parsed, summary); state.completion = reconcileCompletion(parsed.records, completionPayload, parsed.publicationSha256); state.records = parsed.records; state.publication = parsed; state.summary = summary; state.verified = true; updateReviewSurface(); setStatus("Verified position classification workspace available.", "verified"); } catch (error) { state.verified = false; controlsEnabled(false); setStatus(`Review unavailable: ${error.message}`, "failed"); } });
